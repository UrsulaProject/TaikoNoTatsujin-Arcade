...
#import "TaikoNoTatsujinPlus-Prefix.pch"
...
@property (nonatomic, retain) NSString *pathwww;
...
- (void)setUsername:(NSString *)username pw:(NSString *)pw {
...
- (NSMutableURLRequest *)requestByAddingCredentialsToURL:(BOOL)useCredentialsInURL {
    ...
    if([self.filesToUpload count] > 0 || [self.dataToUpload count] > 0) {
        ...
        for(STHTTPRequestFileUpload *fileToUpload in self.filesToUpload) {
            ...
            NSData *data = [NSData dataWithContentsOfFile:fileToUpload.pathwww];
            if(data == nil) continue;
            NSString *fileName = [fileToUpload.pathwww lastPathComponent];
...
- (NSString *)curlDescription {
    ...
    for(STHTTPRequestFileUpload *f in _filesToUpload) {
        NSString *s = [NSString stringWithFormat:@"%@=@%@", f.parameterName, f.pathwww];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
   ...
   //     _completionDataBlock(_responseHeaders,_responseData);
        _completionDataBlock(self.responseHeaders,_responseData);
    ...
    if(_completionBlock)
    {
       ...
       // _completionBlock(_responseHeaders, responseString);
         _completionBlock(self.responseHeaders, responseString);
    }
}
